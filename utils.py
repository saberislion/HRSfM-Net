...
def disp_to_depth(disp, min_depth, max_depth):
    ...
    for i in range(len(disp)):
        scaled_disp = tf.to_float(min_disp) + tf.to_float(max_disp - min_disp) * disp[i]
        depth.append(tf.to_float(1.) / scaled_disp)
    return depth
...
def cam2pixel(cam_coords, proj):
    ...
    pixel_coords = tf.concat([x_n, y_n], axis=1)
    pixel_coords = tf.reshape(pixel_coords, [batch, 2, height, width])
    z_u = tf.reshape(z_u, [batch, 1, height, width])
    return tf.transpose(pixel_coords, perm=[0, 2, 3, 1])

def cam2pixel_depth(cam_coords, proj):
    """Transforms coordinates in a camera frame to the pixel frame.
    ...
    pixel_coords = tf.reshape(pixel_coords, [batch, 2, height, width])
    z_u = tf.reshape(z_u, [batch, 1, height, width])
    return tf.transpose(pixel_coords, perm=[0, 2, 3, 1]), tf.transpose(z_u, perm=[0, 2, 3, 1])
...
def projective_inverse_warp(img, depth, pose, intrinsics, depth1):
    ...
    cam_coords = pixel2cam(depth, pixel_coords, intrinsics)  # (2,4,128,416)
...
def bilinear_sampler(imgs, coords, depth):
    ...
    with tf.name_scope('image_sampling'):
        coords_x, coords_y = tf.split(coords, [1, 1], axis=3)  # coord (2 ,128, 416, 2) coord x (2 ,128, 416, 1)
        inp_size = imgs.get_shape()
        depth_size = depth.get_shape()
        coord_size = coords.get_shape()
        out_size = coords.get_shape().as_list()  # (2 ,128, 416, 2)
        dout_size = depth.get_shape().as_list()
        out_size[3] = imgs.get_shape().as_list()[3]  # 3

        coords_x = tf.cast(coords_x, 'float32')
        coords_y = tf.cast(coords_y, 'float32')

        x0 = tf.floor(coords_x)  # x0 (2 ,128, 416, 1)
        x1 = x0 + 1
        y0 = tf.floor(coords_y)
        y1 = y0 + 1

        y_max = tf.cast(tf.shape(imgs)[1] - 1, 'float32')  # ymax (127)
        x_max = tf.cast(tf.shape(imgs)[2] - 1, 'float32')  # xmax (415)
        zero = tf.zeros([1], dtype='float32')

        x0_safe = tf.clip_by_value(x0, zero, x_max)  # (2 ,128, 416, 1)
        ...
        wt_x0 = x1_safe - coords_x  # (2 ,128, 416, 1)
        wt_x1 = coords_x - x0_safe
        wt_y0 = y1_safe - coords_y
        wt_y1 = coords_y - y0_safe

        ## indices in the flat image to sample from
        dim2 = tf.cast(inp_size[2], 'float32')  # (416)
        dim1 = tf.cast(inp_size[2] * inp_size[1], 'float32')  # (128*416)
        base = tf.reshape(
            _repeat(
        ...
        # base (2 ,128, 416, 1)
        base_y0 = base + y0_safe * dim2  # (2 ,128, 416, 1)
        base_y1 = base + y1_safe * dim2
        idx00 = tf.reshape(x0_safe + base_y0, [-1])  # (2*128*416*1,)
        idx01 = x0_safe + base_y1  # (2 ,128, 416, 1)
        idx10 = x1_safe + base_y0
        idx11 = x1_safe + base_y1

        ## sample from imgs

        imgs_flat = tf.reshape(imgs, tf.stack([-1, inp_size[3]]))  # (2*128*416*1,3)
        depth_flat = tf.reshape(depth, tf.stack([-1, depth_size[3]]))
        imgs_flat = tf.cast(imgs_flat, 'float32')
        depth_flat = tf.cast(depth_flat, 'float32')
        im00 = tf.reshape(tf.gather(imgs_flat, tf.cast(idx00, 'int32')), out_size)  # (2 ,128, 416, 3)
        im01 = tf.reshape(tf.gather(imgs_flat, tf.cast(idx01, 'int32')), out_size)
        im10 = tf.reshape(tf.gather(imgs_flat, tf.cast(idx10, 'int32')), out_size)
        im11 = tf.reshape(tf.gather(imgs_flat, tf.cast(idx11, 'int32')), out_size)


        w00 = wt_x0 * wt_y0  # (2 ,128, 416, 1)
        w01 = wt_x0 * wt_y1
        w10 = wt_x1 * wt_y0
        w11 = wt_x1 * wt_y1
...
def get_cloud(depth, intrinsics):
    ...
    pixel_coords = meshgrid(batch, height, width)
    # Convert pixel coordinates to the camera frame
    # [batch, 4, height, width], com
    cam_coords = pixel2cam(depth, pixel_coords, intrinsics)
    return cam_coords


def camtocloud(cam_coords):
    batch, _, height, width = cam_coords.get_shape().as_list()
    cam_coords = tf.reshape(cam_coords, [batch, 4, -1])
    x_u = tf.slice(cam_coords, [0, 0, 0], [-1, 1, -1])
    y_u = tf.slice(cam_coords, [0, 1, 0], [-1, 1, -1])
